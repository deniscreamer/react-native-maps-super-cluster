  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    onRegionChangeComplete(region) {
        ...
        setTimeout(() => {
            ...
            const newExtent = generateExtentByCount(
                currentZoom,
                this._getDataLength()
            );
    ...
    _getCurrentZoom() {
        return this.mapview && this.mapview.__lastRegion
            ? calcZoom(this.mapview.__lastRegion.longitudeDelta)
            : this.state.region
            ? calcZoom(this.state.region.longitudeDelta)
            : 15;
    }
